...
- **Legacy `.claude/commands/foo.md` = a *default* skill, NOT a `disable-model-invocation` skill.** A bare command file (no frontmatter) lands on the **default row: BOTH can invoke** — Claude *can* auto-load it. It does **not** silently carry `disable-model-invocation: true`. To make `commands/foo.md` behave like `skills/foo/SKILL.md` + `disable-model-invocation: true`, add that flag to the command file's frontmatter (`.claude/commands/` **supports the same frontmatter**). Location never sets the invocation mode; frontmatter does.
- **The ONLY real command-vs-skill difference is the supporting-files directory.** Given identical frontmatter, behavior is identical — same `/name`, same invocation options, same precedence ladder, same `$ARGUMENTS`/`allowed-tools`/`context: fork`. A command is a **lone `.md` file**; a skill is a **directory** (`SKILL.md` + can bundle scripts/templates/reference files). So *"command = a skill with no room for attachments."* The moment a procedure needs a bundled helper script, you need the skill (directory) form — which is why docs say *"Skills are recommended… they support additional features like supporting files."*
...
**Traps:** treating "custom slash command" and "skill" as two mechanisms to choose between (unified — same `/name`, same behavior; only diff = skill's supporting-files dir); assuming a legacy `commands/` file is manual-only (it defaults to auto-invokable like any skill — the flag isn't implied by the folder); "command = manual-only, skill = auto-only" (both invoke modes belong to one skill; the axis is `disable-model-invocation`/`user-invocable`); routing a store-and-surface procedure to a **subagent** because it "also loads on demand" (shared property ≠ shared purpose — subagent = delegate/isolate).
