...
- **Two subsystems, one access — name which resolves first.** A symlinked file can *both* satisfy a path-scoped `paths:` rule (context loads) *and* hit a permission `deny` (hard gate). The **enforcement gate wins before context matters** → the rule never applies to a denied read. When a stem mixes rules-matching with permissions, don't let "the rule matches" hide "the action is blocked." Related: permission symlink checks are **asymmetric** — allow needs link **and** target; deny fires on link **or** target. *(D3)*
- **Batch vs. chain vs. fan-out — two axes:** *dependency* decides ordering (dependent → **chain**; independent → don't), *context-load* decides placement (light/few → **batch** one message; heavy/many → **fan out**). Evaluate **per-item**: "same file / related" is **not** a dependency, and "independent" does **not** force batching. *(D5)*
- **Ordering-dependency ≠ inspection-dependency** — sequential-and-self-carrying → **one ordered prompt**; sequential-but-you-must-see-the-middle → **chain across turns**. Chaining earns its keep only when *you* need the intermediate surfaced. *(D5)*
- **`/clear` vs `/compact` = relevance vs volume** — irrelevant context (topic-switch, dead-end corrections) → **`/clear`**; relevant-but-full (deep in one problem) → **`/compact`** (+ pin findings to a file, since compaction is lossy). Context economy = keep it *relevant*, not *small*. *(D5)*
- **Self-verification is fine when the judge is an external oracle** (tests/build/fixture-diff); only self-*judgment* with no oracle is weak. High-stakes/unattended → escalate to a deterministic gate or independent reviewer. *(D5/D1)*
